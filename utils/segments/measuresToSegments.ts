...
import { SegmentCount, SegmentData, SegmentGenerator } from "./types";
...
export const measuresToSegmentArray = (
  ...
  for (const { notes } of measures) {
    ...
    for (let i = 0; i < notes.length; i++) {
      const { type, annotations } = notes[i];

      // Won't be SegmentBeat if it's dotted (fix)
      const beatPercentage = getNoteDuration(
        type,
        beatNote,
        annotations?.dotted
      ) as SegmentBeat;
